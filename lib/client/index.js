...
module.exports = {
  ...
  async createVersion (nosPath, directx = 'NostaleClientX.exe') {
    const random = [
      0x00,
    ...
    const version = (await Utils.exec(`wmic datafile where name="${sanitizedPath}" get Version`)).toString().trim().split('\n').pop()
  ...
  async createChecksumHash (username, nosPath, directx = 'NostaleClientX.exe', opengl = 'NostaleClient.exe') {
    ...
    if (typeof directx !== 'string' || !directx.length) {
      throw new TypeError('The third argument must be the NosTale DirectX executable filename (if blank will be set to \'NostaleClientX.exe\').')
    }
    if (typeof opengl !== 'string' || !opengl.length) {
      throw new TypeError('The fourth argument must be the NosTale OpenGL executable filename (if blank will be set to \'NostaleClient.exe\').')
    }
